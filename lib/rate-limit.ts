...
export function rateLimit({
  ...
  const existingEntry = requestStore.get(identifier);

  if (!existingEntry || existingEntry.expiresAt <= currentTime) {
  ...
  if (existingEntry.count >= limit) {
    return {
      ...
      remaining: 0,
      retryAfter: Math.max(
        Math.ceil((existingEntry.expiresAt - currentTime) / 1000),
        1,
      ),
    };
  }
  ...
  return {
    success: true,
    remaining: Math.max(limit - updatedEntry.count, 0),
    retryAfter: 0,
  };
}