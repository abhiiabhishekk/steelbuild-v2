...
export default function FeaturedProjects({
  ...
  title = "Built to Perform. Engineered to Last.",
  description = "Discover landmark Pre-Engineered Building projects delivered across India through precision engineering, advanced manufacturing and reliable execution.",
  maxProjects = 6,
...
const showNext = useCallback(() => {
  ...
  useEffect(() => {
    if (activeIndex >= featuredProjects.length) {
      setActiveIndex(0);
    }
  }, [activeIndex, featuredProjects.length]);

  /*
   * Preload previous and next project images.
   * This makes slide switching feel instant.
   */
  useEffect(() => {
    ...
    const imagesToPreload = [
      featuredProjects[previousIndex]?.image,
    ...
    >
      ...
      <Container>
        ...
        >
          ...
          <h2 className="mt-7 text-4xl font-black leading-tight tracking-[-0.045em] text-primary-blue md:text-5xl lg:text-[60px]">
            {title}
          </h2>
        ...
        >
          <motion.article
            drag={featuredProjects.length > 1 ? "x" : false}
  ...
  <motion.div
    ...
    initial={
      prefersReducedMotion
        ? { opacity: 1 }
        : { opacity: 0, x: 35, scale: 1.02 }
    }
    animate={
      prefersReducedMotion
        ? { opacity: 1 }
        : { opacity: 1, x: 0, scale: 1.06 }
    }
    exit={
      prefersReducedMotion
        ? { opacity: 0 }
  ...
  >
    <Image
      ...
      alt={`${activeProject.name} project by Steelbuild Infra Projects`}
      fill
      priority
      fetchPriority="high"
      quality={90}
    ...
    />
            ...
            <div className="pointer-events-none absolute inset-0 opacity-[0.025] mix-blend-soft-light bg-[radial-gradient(circle_at_center,white_0.7px,transparent_0.8px)] bg-[size:4px_4px]" />
            ...
            {featuredProjects.map((project, index) => {
              ...
              return (
                <motion.button
                  ...
                  className={`group/card relative grid min-h-[210px] overflow-hidden rounded-[28px] border bg-white text-left transition-all duration-500 sm:grid-cols-[180px_minmax(0,1fr)] ${
                    selected
                ...
                >
                  ...
                  <div className="relative min-h-[190px] min-w-0 overflow-hidden bg-primary-blue">
                    <Image
                      src={project.image}
                      alt={`${project.name} project by Steelbuild Infra Projects`}
                      fill
                    ...
                    <span className="absolute left-3 top-3 inline-flex max-w-[calc(100%-78px)] items-center whitespace-nowrap rounded-full border border-white/25 bg-black/35 px-3 py-2 text-[9px] font-black uppercase leading-none tracking-[0.12em] text-white shadow-lg backdrop-blur-md">
  {project.category}
</span>

                    {/* <span className="absolute left-4 top-4 inline-flex items-center whitespace-nowrap rounded-full border border-white/25 bg-black/35 px-4 py-2 text-[9px] font-black uppercase leading-none tracking-[0.14em] text-white shadow-lg backdrop-blur-md">
  {project.category}
</span> */}
                  </div>
                  ...
                  <div className="flex min-w-0 flex-col p-5">
                    <div className="min-h-[54px]">
                      <h4 className="line-clamp-2 text-xl font-black leading-[1.3] text-primary-blue">
                        {project.name}
                      </h4>
                    </div>
